...
class TestConfirmInAgentFlow:
    ...
    def test_dangerous_command_denied_stops_that_step(self, tmp_path):
        ...
        script = [
            # 先尝试一个被拒绝的危险命令
            Action(ActionType.TOOL_CALL, "try dangerous", ToolCall("shell", {"cmd": "python -c 'print(1)'"})),
            # 然后用安全命令继续
    ...
    def test_all_allowed_completes_normally(self, tmp_path):
        ...
        script = [
            Action(ActionType.TOOL_CALL, "run python", ToolCall("shell", {"cmd": "python -c 'print(1)'"})),
